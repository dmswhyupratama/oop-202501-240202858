...
public class CashierController {
    ...
    // --- LOGIC VOUCHER (UPDATE: ERROR HANDLING) ---
    private void handleCheckVoucher() {
        ...
        if (voucher != null) {
            ...
            // Feedback Popup Opsional (Bisa dihapus kalau dirasa mengganggu)
            // showAlert("Voucher Valid", "Kode voucher berhasil digunakan!"); 
            ...
            // 2. Tampilkan Alert Error
            showAlert("Voucher Invalid", "Kode voucher tidak ditemukan atau sudah kadaluarsa.");
            
            // 3. Reset input biar user input ulang
    ...
    private void calculateTotal() {
        ...
        // Update Info Text (Tumpuk info voucher + threshold)
        String infoText = "";
        if (activeVoucher != null) {
            infoText += "✅ " + activeVoucher.getDescription();
            // Kembalikan warna hijau jika valid
            view.getLblDiscountInfo().setStyle("-fx-text-fill: green; -fx-font-weight: bold; -fx-font-size: 11px;");
        }
        ...
        if (subtotal > THRESHOLD_LIMIT) {
            ...
            // Kalau dapat threshold, pasti hijau/bagus
            view.getLblDiscountInfo().setStyle("-fx-text-fill: blue; -fx-font-weight: bold; -fx-font-size: 11px;");
        }
        
        // Kalau kosong dan tidak ada error sebelumnya, set text
        if (!infoText.isEmpty()) {
    ...
    // ... Load, Add, Remove, Edit (SAMA SEPERTI SEBELUMNYA) ...
